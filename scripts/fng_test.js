	...
	*/
	fng.selectbox = function(settings){
		var $selector = $(settings.selector),
			...
			$span = $('<span class="sel_val">asdfasd</span>');
		...
		if (settings.url) {
			function readSuccess(data){
				...
				$('option', $selectbox).eq(0).attr("selected", "selected");
			}
			...
			*/
			displayGrid : function(){
				var options = this.options,
	            	$table = $('<table class="table_ty1 ' + options.tableClass + '">'),
	                $thead = $('<thead>'),
	                $colgrp = $('<colgroup>'),
	                $trh = $('<tr>'),
	                $tr = $('<tr>'),
	      ...
	      //                   $trh.append(thString + '>' + header[i][j].title + '</th>');
		            ...
		            if (body) {
		                for (j = 0; j < bodyColCnt; j++) {
							...
							if (body[j].details) {
								...
								if(field)
									td = td + '<td class="details">' + options.dataBody[i][field] + '</td>';
								else
									td = td + '<td class="details"></td>';
							} else {
			                    if(field)
			                    	td = td + '<td class="anc">' + options.dataBody[i][field] + '</td>';
			                   	else
			                   		td = td + '<td class="anc"></td>';
			                }
		                }
		                ...
		                for (var p in options.dataBody[i]) {
		                    ...
		                    td = td + '<td class="anc">' + options.dataBody[i][p] + '</td>';
		                    k++;
		                }
			...
			*/
		    paginationing : function (options, pageclickhdl) {
		        ...
		        if (curpage > pageCnt) {
	                $wrap.append('<a class="pagelink btn_pgprev2" href="' + (start - 1) + '" data-link="' + (start - 1) + '"">이전' + pageCnt + '페이지></a> ');
		        } else {
		            $wrap.append('<a class="btn_pgprev2 disabled" href="javascript:;" data-link="' + (start - 1) + '"">이전' + pageCnt + '페이지></a> ');
		        }

		        if (curpage > 1) {
		            $wrap.append('<a class="pagelink btn_pgprev" href="' + (curpage - 1) + '" data-link="' + (curpage - 1) +'">이전페이지</a> ');
		        } else {
		            $wrap.append('<a class="btn_pgprev disabled" href="javascript:;" data-link="' + (curpage - 1) +'">이전페이지</a> ');
		        }
		        ...
		        if (curpage < totalpages) {
	                $wrap.append(' <a class="pagelink btn_pgnext" href="' + (parseInt(curpage, 10) + 1) + '" data-link="' + (parseInt(curpage, 10) + 1) + '">다음페이지</a> ');
	            } else {
		            $a = $('<a>').text('다음페이지');
		            $wrap.append(' <a class="btn_pgnext disabled" href="javascript:;" data-link="' + (parseInt(curpage, 10) + 1) + '">다음페이지</a> ');
		        }

		        if (start + pageCnt - 1 < totalpages) {
	                $wrap.append('<a class="pagelink btn_pgnext2" href="' + (start + pageCnt) + '" data-link="' + (start + pageCnt) + '">다음' + pageCnt + '페이지</a>');
		        } else {
		            $wrap.append('<a class="btn_pgnext2 disabled" href="javascript:;" data-link="' + (start + pageCnt) + '">다음' + pageCnt + '페이지</a>');
		        }
	            ...
	            //엑셀다운로드 버튼
	            $('<a href="" class="btn_exdown2">엑셀다운로드</a>').appendTo(target);
